...
Otra funcionalidad que no hemos agregado es la de Cancelar Pedido. 
...
Además, se utiliza una API para calcular el costo de envío cuando corresponde (configurable mediante "ORS_API_KEY") y Nominatim para pasar las direcciones en String a coordenadas. Mejora la precisión del precio mostrado al usuario al calcular el costo según dirección y carrier y automatiza el proceso evitando cálculos manuales. Se puede configurar el costo fácilmente.

Cuando el pago ha fallado (por error del cliente desde mercado pago), el cliente puede acceder a un reintento de la compra. Al finalizar exitosamente puede acceder a su orden de compra confirmada con solo un click.

Si un usuario ha decido cambiar los datos de su orden, puede regresar al checkout, donde puede modificar los datos.
